...
class WriteMessageQueue(Thread):
    def __init__(self, bridge, pipeIn):
        ...
        self._exit = False
        self.Bridge = bridge
        self.ToXTMF = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        self.ToXTMF.connect(pipeIn)
            ...
            while not self._exit:
                ...
                if msg is not None:
                    for subMessage in msg:
                        #msg is of type array
                        code = subMessage.typecode
                        elements = subMessage.buffer_info()[1]
                        element_size = subMessage.itemsize
                        buf = bytearray(elements * element_size)
                        current_pos = 0
                        for el in xrange(0, elements):
                            struct.pack_into(code, buf, current_pos, subMessage[el])
                            current_pos += element_size
                        self.ToXTMF.send(buf)
                else:
                    return
        except Exception, e:
            self.Bridge.WriteToConsole("We had an exception while writing!")
            self.Bridge.WriteToConsole(e.message)
        return
...
class XTMFBridge:
    ...
    def __init__(self):
        ...
        print "Running Linux Modeller Bridge"
        self.FromXTMF = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        self.FromXTMF.connect(pipeOut)
    ...
    def ReadString(self):
        ...
        if length <= 0:
            return ""
        try:
            return self.FromXTMF.recv(length)
        except Exception, e:
            self.WriteToConsole("Failed to read in a string!\r\n" + e.message)
            return None

    def ReadIntoBuffer(self, size):
        buf = bytearray(size)
        self.FromXTMF.recv_into(buf, size)
        return buf

    def ReadByte(self):
        val = struct.unpack('B', self.ReadIntoBuffer(1))
        return val[0]
    
    def ReadInt(self):
        val = struct.unpack('i', self.ReadIntoBuffer(4))
        return val[0] 
    ...
    def ExecuteModule(self, useBinaryParameters):
        ...
        try:
            ...
            self.WriteToConsole("Starting to run " + macroName)
            ...
            self.WriteToConsole("ERROR: " + inst.message)
            if timer != None:
                timer.stop()
