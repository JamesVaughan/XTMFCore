...
namespace TMG.GTAModel.DataResources
{
    ...
    public class TransitEgressGoStationUtility : IDataSource<SparseTwinIndex<Tuple<IZone[], IZone[], float[]>>>
    {
        ...
        {
            float v = ComputeAccessUtility(interchange, d, out float accessTravelTime);
            // our total travel time / distance is the egress time plus the time it takes to get to the station to begin with
            distance = egressTime[accessStationIndex][o] + accessTravelTime;
            ...
            if (distance < ComputeWeightedTimeWithoutRail(d, o))
            {
        ...
        private void ComputeEgressStation(int interchange, int origin, int[] egressZones, float[] trains, out float egressUtility, out float egressTime, out int egressZone)
        {
            ...
            float maxEgressTime = ComputeWeightedTimeWithoutRail(origin, interchange);
        ...
        {
            if (!PremiumTransitNetwork.GetAllData(origin, destination, TimeOfDay, out Time ivtt, out Time walk, out Time wait, out Time boardings, out float cost) | ivtt <= Time.Zero)
            {
        ...
        private float ComputeEgressStationUtility(int interchange, int egress, int destination)
        {
            ...
            var goWalk = GoTransitNetwork.WalkTime(interchange, egress, TimeOfDay);
