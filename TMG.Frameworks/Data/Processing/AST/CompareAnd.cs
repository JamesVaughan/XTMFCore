        ...
        public override ComputationResult Evaluate(ComputationResult lhs, ComputationResult rhs)
        {
            ...
            if (lhs.IsValue)
            {
                if (rhs.IsVectorResult)
                {
                    ...
                    VectorHelper.FlagAnd(flat, lhs.LiteralValue, rhs.VectorData.GetFlatData());
                ...
                else
                {
                    ...
                    VectorHelper.FlagAnd(retMatrix.GetFlatData(), lhs.LiteralValue, rhs.OdData.GetFlatData());
            ...
            else if (rhs.IsValue)
            {
                if (lhs.IsVectorResult)
                {
                    ...
                    var flat = retVector.GetFlatData();
                    VectorHelper.FlagAnd(flat, lhs.VectorData.GetFlatData(), rhs.LiteralValue);
                ...
                else
                {
                    ...
                    VectorHelper.FlagAnd(retMatrix.GetFlatData(), lhs.OdData.GetFlatData(), rhs.LiteralValue);
                    return new ComputationResult(retMatrix, true);
                }
            }
            else
            {
                if (lhs.IsVectorResult || rhs.IsVectorResult)
                {
                    if (lhs.IsVectorResult && rhs.IsVectorResult)
                    {
                        ...
                        VectorHelper.FlagAnd(retMatrix.GetFlatData(), 0, lhs.VectorData.GetFlatData(), 0, rhs.VectorData.GetFlatData(), 0, retMatrix.GetFlatData().Length);
                    ...
                    else if (lhs.IsVectorResult)
                    {
                        ...
                        if (lhs.Direction == ComputationResult.VectorDirection.Vertical)
                        {
                            ...
                            {
                                VectorHelper.FlagAnd(flatRet[i], flatRhs[i], flatLhs[i]);
                            });
                        }
                        else if (lhs.Direction == ComputationResult.VectorDirection.Horizontal)
                        {
                            System.Threading.Tasks.Parallel.For(0, flatRet.Length, i =>
                            {
                                VectorHelper.FlagAnd(flatRet[i], 0, flatLhs, 0, flatRhs[i], 0, flatRet[i].Length);
                            });
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (rhs.Direction == ComputationResult.VectorDirection.Vertical)
                        {
                            System.Threading.Tasks.Parallel.For(0, flatRet.Length, i =>
                            {
                                VectorHelper.FlagAnd(flatRet[i], flatLhs[i], flatRhs[i]);
                            });
                        }
                        else if (rhs.Direction == ComputationResult.VectorDirection.Horizontal)
                        {
                            System.Threading.Tasks.Parallel.For(0, flatRet.Length, i =>
                            {
                                VectorHelper.FlagAnd(flatRet[i], 0, flatRhs, 0, flatLhs[i], 0, flatRet[i].Length);
                            });
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    VectorHelper.FlagAnd(retMatrix.GetFlatData(), lhs.OdData.GetFlatData(), rhs.OdData.GetFlatData());
                    return new ComputationResult(retMatrix, true);
                }
            }
        }
