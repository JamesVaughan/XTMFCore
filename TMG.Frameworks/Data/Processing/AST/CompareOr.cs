        ...
        public override ComputationResult Evaluate(ComputationResult lhs, ComputationResult rhs)
        {
            ...
            if (lhs.IsValue)
            {
                if (rhs.IsVectorResult)
                {
                    ...
                    VectorHelper.FlagOr(flat, lhs.LiteralValue, rhs.VectorData.GetFlatData());
                ...
                else
                {
                    ...
                    VectorHelper.FlagOr(retMatrix.GetFlatData(), lhs.LiteralValue, rhs.OdData.GetFlatData());
            ...
            else if (rhs.IsValue)
            {
                if (lhs.IsVectorResult)
                {
                    ...
                    var flat = retVector.GetFlatData();
                    VectorHelper.FlagOr(flat, lhs.VectorData.GetFlatData(), rhs.LiteralValue);
                ...
                else
                {
                    ...
                    VectorHelper.FlagOr(retMatrix.GetFlatData(), lhs.OdData.GetFlatData(), rhs.LiteralValue);
                    return new ComputationResult(retMatrix, true);
                }
            }
            else
            {
                if (lhs.IsVectorResult || rhs.IsVectorResult)
                {
                    if (lhs.IsVectorResult && rhs.IsVectorResult)
                    {
                        ...
                        VectorHelper.FlagOr(retMatrix.GetFlatData(), 0, lhs.VectorData.GetFlatData(), 0, rhs.VectorData.GetFlatData(), 0, retMatrix.GetFlatData().Length);
                    ...
                    else if (lhs.IsVectorResult)
                    {
                        ...
                        if (lhs.Direction == ComputationResult.VectorDirection.Vertical)
                        {
                            ...
                            {
                                VectorHelper.FlagOr(flatRet[i], flatLhs[i], flatRhs[i]);
                            });
                        }
                        else if (lhs.Direction == ComputationResult.VectorDirection.Horizontal)
                        {
                            System.Threading.Tasks.Parallel.For(0, flatRet.Length, i =>
                            {
                                VectorHelper.FlagOr(flatRet[i], 0, flatLhs, 0, flatRhs[i], 0, flatRet[i].Length);
                            });
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        else if (rhs.Direction == ComputationResult.VectorDirection.Horizontal)
                        {
                            System.Threading.Tasks.Parallel.For(0, flatRet.Length, i =>
                            {
                                VectorHelper.FlagOr(flatRet[i], 0, flatRhs, 0, flatLhs[i], 0, flatRet[i].Length);
                            });
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    VectorHelper.FlagOr(retMatrix.GetFlatData(), lhs.OdData.GetFlatData(), rhs.OdData.GetFlatData());
                    return new ComputationResult(retMatrix, true);
                }
            }
        }
